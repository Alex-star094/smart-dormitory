...
Page({
  ...
  addToBlacklist: function () {
    ...
    getApp().request({
      ...
      method: 'POST',
      header: { 'Content-Type': 'application/json' }, // 改为JSON格式请求头
      data: { // 保持参数名不变，直接传递JSON对象（无需表单编码）
        name: userName,
      ...
      success: (res) => {
        if (res.confirm) {
          getApp().request({
            ...
            method: 'PUT',
            header: { 'Content-Type': 'application/json' }, // 表单格式
            data: {
              status: "removed",
  ...
  onEffectiveToInput: function (e) { const inputDate = e.detail.value.trim();
    if (!inputDate) {
